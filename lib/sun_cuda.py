...
def illumination(dem, cell_size, latitude, month):
    cuda_dem = nbcuda.to_device(dem)
    cuda_illum = nbcuda.to_device(np.zeros_like(dem))

    threads = (16,16)
    blocks = (512//16, 512//16)
    ...
    for minutes in range(0, 1440, 60):
        ...
        if sundir[1] > 0.0:
            ...
            for r in range((cuda_dem.shape[0]+511)//512):
                for c in range((cuda_dem.shape[1]+511)//512):
                    cuda_exposure[blocks, threads](cuda_dem, cell_size, sundir, cuda_illum, r*512, c*512)

    return cuda_illum.copy_to_host() / max_illum
...
@nbcuda.jit
def cuda_exposure(dem, cell_size, sundir, out, r0, c0):
    r, c = nbcuda.grid(2)
    r+=r0
    c+=c0
